...
### Phase 3: 実行管理（Phase 2完了後に自動実行）

| # | エージェント名 | 役割 | 出力ファイル |
|---|--------------|------|-------------|
| ⑩ | 実行管理PMO (Execution Manager) | 全タスク整理・担当振り分け・優先順位・KSF/KPI・財務管理を統合 | `projects/[PJ]/10_task_management.xlsx` + `projects/[PJ]/10_kpi_dashboard.xlsx` + `projects/[PJ]/10_financial_plan.xlsx` |
...
### Phase 3 実行フロー（Phase 2完了後に自動実行）

1. Phase 1 + Phase 2の全出力を読み込む
2. ⑩実行管理PMOが以下の3つのxlsxファイルを生成する：
   - `10_task_management.xlsx` — タスク管理表
   - `10_kpi_dashboard.xlsx` — KSF/KPIダッシュボード
